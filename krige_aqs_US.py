...
sPM_fn = '/home/kaodell/nasa_fires/Lipner_fullUSkrige/datafiles/krigedPM25_06-19_v2/sfc_pm'+str(year)[-2:]+'_v2.npz'
...
############################################################################################################
print('kriging ',year,', using sPM file:',sPM_fn)
fn = sPM_fn
f = np.load(fn,'r')
sPM = f['sPM']  
lons = f['lons']
...
############################################################################################################
#sPMin is sPM for sites inside glon and glat grid (used for kriging and stats)
#sPMout sPM for sites close to glon and glat, but not inside (used for kriging, but not stats)
...
#for si in [113]: # for testing code
    ...
    sIDs_TEMP = np.hstack([isIDs_TEMP, osIDs])
    
    #remove monitors at same site
    dups = np.where( sIDs_TEMP == siteID)[0]
    ...
    kslope, kintercept, krvalue, kpvalue, kstderr = linregress( xout,yout )
    ksRSQ[si] = krvalue**2.
    ksslope[si] = kslope
...
#for ti in range(1): # for testing
    ...
    if len(INDS[0]) <= 0.1 * (NIS + NOS):
        #print('less than 10% of sites available on DOY ',ti)
        #print NED
        NED += 1
        EDind.append(ti)
    datatemp2 = datatemp[INDS]
    templon = lonK[INDS]
    templat = latK[INDS]
    
    ok = OrdinaryKriging( templon, templat, datatemp2, variogram_model='spherical', \
       	variogram_parameters = vgp, verbose=False, enable_plotting=False)
    z, ss = ok.execute( 'points', glons.reshape((1,-1)), glats.reshape((1,-1)) )
    kPM[ti, :, :] = z.reshape( (NX, NY) )

print('number of days with <10% of sites reporting: ',NED)
...
np.savez(f, alldays=alldays, kPM=kPM,monIDs=monIDs,siteIDs=siteIDs,glats=glats,glons=glons,sPMin=sPMin,\
	sPMout=sPMout,ilon=ilon,ilat=ilat,olat=olat,olon=olon,slope=slope,kPM_site=kPM_site,\
	 ...
	 intercept=intercept,rvalue=rvalue,pvalue=pvalue,stderr=stderr,ksRSQ=ksRSQ,ksslope=ksslope,ksMB=ksMB,ksMAE=ksMAE,knobs=knobs,rsq=rsq,MB=MB,MAE=MAE)
f.close()

print(year,' done, data saved')
